...
export default function CommunityPage() {
  ...
  const handleDispatch = () => {
    ...
    toast({
      ...
      description: "Your diagnostic thesis has been broadcast to the global network.",
    });
  };

  const handleAddSymbol = () => {
    const sym = prompt("Enter asset symbol:");
    if (sym && sym.trim()) {
      const upper = sym.trim().toUpperCase();
      if (!attachedSymbols.includes(upper)) setAttachedSymbols([...attachedSymbols, upper]);
    }
  };
  ...
  return (
    <main className="min-h-screen bg-black text-white selection:bg-indigo-500 font-body">
      <div className="grid min-h-screen grid-cols-1 xl:grid-cols-[290px_minmax(0,1fr)_330px]">
        ...
        {/* CENTER */}
        <section className="min-w-0 bg-transparent flex flex-col h-screen overflow-hidden">
          {/* HEADER */}
          <header className="h-32 border-b border-white/10 bg-black/40 backdrop-blur-md px-8 flex items-center justify-between shrink-0 sticky top-0 z-50">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col text-left">
                <span className="text-[24px] font-black tracking-[0.3em] text-white uppercase leading-none">Global Network</span>
                ...
                <div className="bg-[#070b16] p-6 text-white">
                  ...
                  <div className="flex items-center justify-between mt-6 pt-4 border-t border-white/5">
                    <div className="flex items-center gap-6">
                      <button onClick={handleAddSymbol} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-indigo-400 transition-colors">
        ...
        <aside className="border-l border-white/8 bg-black">
          <div className="sticky top-0 h-screen flex flex-col">
            <div className="p-6 flex items-center gap-3 border-b border-white/5 shrink-0">
              ...
              <div className="text-[12px] font-black tracking-[0.3em] text-indigo-500 uppercase">Diagnostic Pacing</div>
            </div>
            <ScrollArea className="flex-1 min-h-0">
                  ...
                  <div className="space-y-5">
                    {[
                      ...
                      <div key={i} className="flex items-center gap-4 p-2 rounded-xl hover:bg-white/5 transition-all cursor-pointer group">
                        <AvatarBadge label={hub.name[0]} online={hub.active} />
        ...
        <DialogContent className="max-w-4xl bg-[#070b16] border-white/10 text-white rounded-[32px] overflow-hidden p-0 shadow-[0_0_100px_rgba(0,229,255,0.15)]">
          <div className="p-8 h-full flex flex-col">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8 flex-1">
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input 
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm outline-none focus:border-cyan-500/50"
                  ...
                  <div className="space-y-1 mt-3 max-h-[120px] overflow-auto pr-2 custom-scrollbar">
                    {filteredCatalog.map(item => (
                      <button 
                        ...
                        className={`w-full text-left px-3 py-2 rounded-lg text-[11px] font-black transition-all uppercase tracking-widest ${selectedChartSymbol === item.symbol ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30' : 'hover:bg-white/5 text-white/40'}`}
                      >
                        {item.display}
                      </button>
                    ))}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-white/40 flex items-center gap-2"><Type className="w-3 h-3" /> Markup (Write on Chart)</label>
                  <textarea 
                    className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-xs font-black uppercase tracking-widest outline-none focus:border-cyan-500/50 h-32 resize-none text-white placeholder:text-white/10"
              ...
              <div className="relative rounded-2xl border border-white/5 bg-black/40 p-6 overflow-hidden flex flex-col min-h-[400px]">
                <div className="absolute top-6 left-6 z-20 px-3 py-1 rounded-full bg-cyan-500/20 border border-cyan-500/40 text-[9px] font-black text-cyan-300 uppercase tracking-widest">
                  SNAPSHOT PREVIEW: {selectedChartSymbol}
                </div>
                ...
                {annotationText && (
                  <div className="absolute bottom-16 right-12 z-30 transform rotate-[-2deg] animate-in zoom-in-95 duration-200 max-w-[220px]">
                    <div className="bg-yellow-400 text-black px-4 py-3 rounded-sm shadow-2xl font-mono text-[14px] font-black border-b-2 border-black/20 leading-tight">
                      {annotationText}
                    </div>
            ...
            <DialogFooter className="mt-8 pt-6 border-t border-white/5">
              ...
              <Button onClick={() => {
                setActiveAttachment({ symbol: selectedChartSymbol, annotation: annotationText });
                setIsChartModalOpen(false);
              }} className="bg-cyan-500 hover:bg-cyan-400 text-black font-black uppercase text-[10px] tracking-widest px-8 h-11 rounded-xl shadow-[0_0_30px_rgba(0,229,255,0.4)]">
                Map & Capture →
              </Button>
            </DialogFooter>
          </div>
